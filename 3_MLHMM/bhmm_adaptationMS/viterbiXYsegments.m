function [s_t, log_trajectory_probability] = viterbiXYsegments(o_t, segments, models)
...
log_delta_it = zeros(N, T, 'double');
Phi_it = zeros(N, T, 'int32');
...
for i = 1:N
  ...
  log_delta_it(i,1) = models{1}.logPi(i) + log_emission_probabilities(1,i);
  Phi_it(i,1) = 0;
end
...
for t = 2:T
  for j = 1:N
    ...
    [log_valmax, argmax] = max(log_delta_it(:,t-1) + models{1}.logTij(:,j));
    log_delta_it(j,t) = log_valmax + log_emission_probabilities(t,j);  

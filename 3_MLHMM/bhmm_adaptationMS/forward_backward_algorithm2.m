function [log_alpha_ti, log_beta_ti] = forward_backward_algorithm2(o_t, model, log_rho_i)
...
for i = 1:N
  log_alpha_ti(1,i) = log_rho_i(i) + model.states{i}.log_emission_probability(o_t(1,1)) ...
                                    + model.states{i}.log_emission_probability(o_t(2,1));  
end

% Use recurrence relation to compute log probabilities for times 2..T.
for t = 2:T
  for j = 1:N
    log_alpha_ti(t,j) = logsum(log_alpha_ti(t-1,:) + model.logTij(:,j)') + model.states{j}.log_emission_probability(o_t(1,t)) ...
                                                                    + model.states{j}.log_emission_probability(o_t(2,t));    
  end
end
...
for t = (T-1):-1:1
  for i = 1:N
    log_P_o = zeros(1,N);
    for j = 1:N
      log_P_o(1,j) = model.states{j}.log_emission_probability(o_t(1,t+1)) + model.states{j}.log_emission_probability(o_t(2,t+1));
    end
